...
defmodule Solution do
  ...
  def juggle(lines) do
    ...
    [a, b] =
      0..(@rounds - 1)
      |> Enum.reduce(monkeys, fn _, acc -> proc_round(acc) end)
  ...
  defp proc_monkey({_name, items, op, test, then, _checks}),
    do: Enum.map(items, fn item -> {item |> op.(), item |> op.() |> test.() |> then.()} end)
  ...
  defp monkey_op_to_fn("* old"), do: fn x -> floor(x * x / 3) end

  defp monkey_op_to_fn(<<op::binary-size(1), " ", num::binary>>),
    do: fn x -> floor(apply(Kernel, :"#{op}", [x, String.to_integer(num)]) / 3) end

  defp monkey_test_to_fn(num_str), do: fn x -> rem(x, String.to_integer(num_str)) == 0 end
end

IO.puts("The level of monkey business after 20 rounds is #{Solution.juggle(lines)}")
